...
def create_text(already:str, new:str):
    result = ''
    for char in new:
        if char in already:
...
def write_note(x:int, y:int, text:str):
    global notes
...
# TODO: the hint can reveal a word that the user already entered in which case that hint will be wasted
# make it in such a way that it randomly selects from a list of EMPTY entries OR it corrects teh user if the user entered a wrong value
def hint_clicked():
...
def restart_clicked():
    global entries, hints_left, seconds
    restart = messagebox.askyesno(title="Confirmation", message="Are you sure you want to restart this game?")
    if restart:
        # deleting all the entries made by the user
        for entry in entries:
            # from oth index till the end
            entry.delete(0, 'end')
        # deleteing all the notes
        for note in notes:
            canvas.itemconfig(note[0], text='')
        # resetting hints left and the timer
        hints_left = 3
        seconds = 0
...
#*----------RELATED TO PAUSE BUTTON------------#
pause = False # start the game with pause = False
def pause_clicked():
    global timer_on, seconds, pause, pause_label, buttons_list
    # if the game wasn't paused already, we want to pause it now
    if not pause:
        # pausing the timer
        timer_on = False
        # chaging pause's state
        pause = True
        # creating a label which will overlap the whole grid so that the user won't be able to see it
        pause_label = Label(text="The game is paused ⏸", font=("apple chancery", 61), bg="#d0f7dc", relief='ridge', bd=15 ,height=9, padx=160)
        pause_label.grid(row=0, column=0, columnspan=9, rowspan=9)
        # disabling all the buttons except pause/play button so that the user won't be able to do anything while the game is on pause
        for button in buttons_list:
        ...
        # resuming the timer and calling it
        timer_on = True
        update_timer()
        pause = False
        # deastroying this so that the user will be able to see the sudoku board again
        pause_label.destroy()
        # enabling all the buttons that were previosly disabled
        for button in buttons_list:
            button['state'] = NORMAL
...
validate_button.place(x=1000, y=300)
...
restart_button.place(x=1000, y=200)

# these 3 are small buttons at the top
pause_button = Button(text='⏯', command=pause_clicked, highlightbackground=BUTTON_BG, font=small_button_font, width=1)
pause_button.place(x=1000, y=40)

theme_button = Button(text="🎨", font=small_button_font, width=1, highlightbackground=BUTTON_BG, command=change_theme)
theme_button.place(x=1075, y=40)

instructions_button = Button(text="📜", font=small_button_font, command=instructions_clicked, width=1, highlightbackground=BUTTON_BG)
instructions_button.place(x=1150, y=40)


# saving all the buttons in this list so that we can destroy it by iterating through it when the user hit the quit button
buttons_list = [submit_button, note_button, hint_button, validate_button, restart_button, instructions_button, theme_button, quit_button, pause_button]
